...
app.get('/login', function(req,res,next) {
  ...
  MongoClient.connect(url, function(err, db) {
    ...
    var query = { username: req.query.username };
     ...
     {
      if (results.length>0)
        res.render('profile.html', {username : results[0].username, type: doc.type, name: doc.name, surname: doc.surname, adress: doc.adress, email: doc.email, gender: doc.ender, password: doc.password});
      dbo.close(); 
      });
